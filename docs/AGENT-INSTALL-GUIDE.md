...
## 12. 向用户介绍 CFBrain 的能力

**安装和验收完成后，你必须主动向用户介绍 CFBrain 能做什么。** 用户（真人）不会去读 `--help`，他们需要你用人话告诉他们。
...
然后用**口语化、分场景**的方式向用户介绍。以下是参考话术，根据实际安装情况调整（比如没装飞书就不提飞书部分）：

---

### 参考话术

> 🎉 **CFBrain 已经装好了！** 简单给你介绍一下它能做什么：
>
> **📝 记东西**
> 你可以让我帮你记录任何信息——人物、公司、会议纪要、项目进展、灵感想法。每条信息会按分类归档，还能互相关联。比如：
> - "帮我记一下张三是 XX 公司的 CTO，我们上周聊过 AI 落地"
> - "记录一下今天会议的要点：1. ... 2. ..."
>
> **🔍 找东西**
> 记过的东西都能搜出来。支持关键词搜索，配了 API key 后还能用自然语言提问：
> - "张三是做什么的？"
> - "上周关于融资的会议都聊了什么？"
>
> **📄 吃文件**
> 可以把 PDF、PPT、Markdown 笔记批量导入。大文件会自动拆分分析：
> - "把这个 PDF 录入知识库"
> - "导入我 ~/notes 目录下的所有笔记"
>
> **🔗 知识图谱**
> 信息之间会自动建立关联——比如一个人属于哪家公司、参加了哪些会议。你可以问：
> - "跟张三相关的所有信息"
> - "XX 公司都有哪些人？"
>
> **📊 分类管理**
> 默认有 10 种分类（人物、公司、会议、项目等），你可以随时加减：
> - "加一个'论文'分类"
> - "把'交易'分类删掉"
>
> **🔒 数据安全**
> 所有数据都在你本地，不会自动上传到任何地方。API key 也不会进 git。
>
> **☁️ 飞书同步**（如果装了飞书集成）
> 可以把知识库同步到飞书 Wiki，在飞书里直接看：
> - "把所有页面推送到飞书"
> - "看看飞书同步状态"
>
> **💡 怎么开始？**
> 最简单的方式：直接告诉我你想记什么，我来操作。不需要记任何命令。

---

### 介绍时的注意事项

- **根据实际情况裁剪**：没装飞书就不提飞书；没配 API key 就说明自然语言搜索需要配 key
- **强调"告诉我就行"**：用户不需要学命令，Agent 会翻译成 CLI 操作
- **提到分类**：告诉用户当前有哪些分类（执行 `types list` 获取），让他们知道可以自定义
- **不要甩命令**：用户是人，不是终端。用场景和例句，不要列命令行参数
