...
1. **Compiled truth is NOT exempt from citations.** "Pedro co-founded Brex" in the synthesis section needs `[Source: ...]` just as much as any other entry does. Most agents skip citations above the bar.
...
5. **Every evidence entry needs a source.** Every append to the page carries provenance. An entry without a source is an orphan fact.
...
4. Check evidence entries on 3 random pages. Each entry should have a source citation with date and context.
