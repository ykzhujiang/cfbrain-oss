...
on new_meeting_transcript(meeting):
    ...
    for person in meeting.attendees + meeting.mentioned_people:
        page = gbrain get <person_slug>
        updated_truth = integrate(page.compiled_truth,
            "Met in '{meeting.title}' on {date}. Key points: ...")
        gbrain put <person_slug> --content updated_truth
    ...
    for company in meeting.mentioned_companies:
        page = gbrain get <company_slug>
        updated_truth = integrate(page.compiled_truth,
            "Discussed in '{meeting.title}': {what_was_said}")
        gbrain put <company_slug> --content updated_truth
...
2. **Entity propagation is the step most agents skip.** A meeting is NOT fully ingested until every attendee's page, every mentioned person's page, and every company's page has been updated. The meeting page alone is useless without propagation.
...
2. For each attendee, run `gbrain get <attendee_slug>`. Check that their page has been updated with information referencing the meeting with specific insights (not just "attended meeting").
3. Pick a company mentioned in the meeting. Run `gbrain get <company_slug>`. Confirm the page was updated with what was discussed about the company.
