...
RESPOND with brain context (every answer is better with context)
  │
  ▼
WRITE: update brain pages (new info → compiled truth)
...
on_message(text):
  ...
  if response_contains_new_info(response):
    for entity in mentioned_entities:
      page = gbrain_get(entity.slug)
      updated_truth = integrate_new_info(page.compiled_truth, {
        date: today,
        summary: "Discussed {topic}",
        source: "[Source: User, conversation, {date}]"
      })
      gbrain_put(entity.slug, { compiled_truth: updated_truth })
