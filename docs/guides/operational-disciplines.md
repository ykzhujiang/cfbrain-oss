...
on every_inbound_message(message):
    ...
    for entity in entities:
        ...
        if existing:
            # Update entity page with new signal from the message
            page = gbrain get <entity_slug>
            updated_truth = integrate(page.compiled_truth, what_was_said, source)
            gbrain put <entity_slug> --content updated_truth
...
on nightly_schedule("02:00"):
    ...
    # 5c: Memory consolidation -- update compiled truth from recent activity
    for page in stale_pages(older_than="7d"):
        page = gbrain get <page.slug>
        if page.has_new_activity_since_last_consolidation:
            # Re-synthesize compiled truth from accumulated evidence
            updated_truth = consolidate(page.compiled_truth, page.recent_activity)
...
3. **Signal detection must fire on EVERY message.** Not just messages that look important. The user says "I talked to Pedro yesterday about the board seat" in passing -- that's an update to Pedro's page, a potential update to his State section, and a signal about the board. If the agent doesn't catch it, the system is broken.
...
1. Send a message mentioning a person with a brain page. Confirm the agent detects the entity and updates their page (`gbrain get <slug>`).
