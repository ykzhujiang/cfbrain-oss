...
`);
  ...
  test('chunks compiled_truth correctly', async () => {
...
`);
  ...
  test('imports file with minimal content (no timeline separator)', async () => {
...
`);
    ...
    if (chunkCall) {
      const chunks = chunkCall.args[1];
      expect(chunks.every((c: any) => c.chunk_source === 'compiled_truth')).toBe(true);
    }
  });
