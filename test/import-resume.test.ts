...
const TEST_DIR = join(tmpdir(), `cfbrain-test-checkpoint-${process.pid}`);
const CHECKPOINT_PATH = join(TEST_DIR, 'import-checkpoint.json');

describe('import resume checkpoint', () => {
  afterEach(() => {
    // Clean up entire temp dir after each test
