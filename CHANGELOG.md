...
- **Git auto-push no longer hangs on brain.pglite.** The `brain.pglite/` directory (PGLite database files) was accidentally tracked by Git LFS, causing `git lfs push` to hang indefinitely during auto-push. Fixed by removing `brain.pglite/` from git tracking, adding it to `.gitignore`, and removing the LFS filter rule from `.gitattributes`. The local database is unaffected — only the git sync behavior is fixed.

### Changed

- **Data repo `.gitignore` updated.** Added `brain.pglite/`, `brain.pglite-backup-*/`, `brain.pglite-corrupted-*/`, `brain.pglite.bak.*/` to prevent future accidental commits of database files.
- **Data repo `.gitattributes` cleaned.** Removed `brain.pglite/** filter=lfs` rule. Only `raw/**` files use LFS now.
