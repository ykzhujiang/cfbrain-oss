...
- **发言归属是最隐蔽的错误**（2026-04-17 教训）：转发消息中 `[某好友] [Image]` 被错误归为"<OWNER>朋友圈"，因为没有逐条检查发送者。归属错误会污染知识库的可信度
- **纯文字回复 ≠ 图片消息**（2026-04-17 追加教训）：<OWNER>对该好友转发内容的一句文字附和，被误当成又发了一张图。没有逐条对照消息元数据（[Image] 标记）和实际图片附件，导致多算了一张图、归属错误。**必须建立消息与附件的一一对应表后再写 content**
