...
#   generator         — generator, evaluator, or planner
#   $CFBRAIN_HOME    — Monorepo root, e.g. $HOME/.news-briefing
#   $CFBRAIN_HOME-generator — Role clone root, e.g. $HOME/.news-briefing-generator
#   3497    — Role port, e.g. 3469
...
#
# Monorepo: AGENTS.md lives at generator/AGENTS.md (in place, no cp needed).
# Git operations use REPO_ROOT (the clone root), agent runs in WORK_DIR (role subdir).

set -euo pipefail

LOCK_DIR="/tmp/<AGENT>-generator.lock"
...
REPO_ROOT="$CFBRAIN_HOME-generator"
WORK_DIR="$CFBRAIN_HOME-generator/generator"
AGENT_ROLE="generator"
LOG_DIR="${WORK_DIR}/logs"
TIMEOUT=3600

LOG_FILE="${LOG_DIR}/generator-$(date +%Y%m%d).log"
mkdir -p "$LOG_DIR"

# --- Log rotation: remove logs older than 30 days ---
find "$LOG_DIR" -name "generator-*.log" -type f -mtime +30 -delete 2>/dev/null || true

log() {
    echo "[$(date '+%Y-%m-%d %H:%M:%S')] [generator] $*" >> "$LOG_FILE"
}
...
if ! mkdir "$LOCK_DIR" 2>/dev/null; then
    if [ -d "$LOCK_DIR" ]; then
        ...
        if [ "$lock_age" -gt 600 ]; then
            ...
            log "SKIP: another generator instance is running (${lock_age}s)"
            exit 0
        fi
    fi
fi
...
# monorepo: AGENTS.md is at generator/AGENTS.md in place, no cp needed
...
timeout "$TIMEOUT" "$OPENCODE" run \
    ...
    "Execute your standard generator cycle." \
...
if [ -n "$dirty_files" ]; then
    ...
    if [ "$dirty_count" -le 20 ]; then
        ...
        if cd "$REPO_ROOT" \
            && git add shared/ main/ generator/ evaluator/ planner/ launchd/ .gitignore 2>/dev/null; true \
            && git commit -m "auto-recover: generator session orphaned changes (${dirty_count} files)" \
            && git push; then
...
latest_summary=$(ls -t "$REPORTS_DIR"/generator-*.md 2>/dev/null | head -1)
