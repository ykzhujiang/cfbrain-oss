...
#   evaluator         — generator, evaluator, or planner
#   $CFBRAIN_HOME    — Monorepo root, e.g. $HOME/.news-briefing
#   $CFBRAIN_HOME-evaluator — Role clone root, e.g. $HOME/.news-briefing-generator
#   3498    — Role port, e.g. 3469
...
#
# Monorepo: AGENTS.md lives at evaluator/AGENTS.md (in place, no cp needed).
# Git operations use REPO_ROOT (the clone root), agent runs in WORK_DIR (role subdir).

set -euo pipefail

LOCK_DIR="/tmp/<AGENT>-evaluator.lock"
...
REPO_ROOT="$CFBRAIN_HOME-evaluator"
WORK_DIR="$CFBRAIN_HOME-evaluator/evaluator"
AGENT_ROLE="evaluator"
LOG_DIR="${WORK_DIR}/logs"
TIMEOUT=1800

LOG_FILE="${LOG_DIR}/evaluator-$(date +%Y%m%d).log"
mkdir -p "$LOG_DIR"

# --- Log rotation: remove logs older than 30 days ---
find "$LOG_DIR" -name "evaluator-*.log" -type f -mtime +30 -delete 2>/dev/null || true

log() {
    echo "[$(date '+%Y-%m-%d %H:%M:%S')] [evaluator] $*" >> "$LOG_FILE"
}
...
if ! mkdir "$LOCK_DIR" 2>/dev/null; then
    if [ -d "$LOCK_DIR" ]; then
        ...
        if [ "$lock_age" -gt 600 ]; then
            ...
            log "SKIP: another evaluator instance is running (${lock_age}s)"
            exit 0
        fi
    fi
fi
...
# monorepo: AGENTS.md is at evaluator/AGENTS.md in place, no cp needed
...
timeout "$TIMEOUT" "$OPENCODE" run \
    ...
    "Execute your standard evaluator cycle." \
...
if [ -n "$dirty_files" ]; then
    ...
    if [ "$dirty_count" -le 20 ]; then
        ...
        if cd "$REPO_ROOT" \
            && git add shared/ main/ generator/ evaluator/ planner/ launchd/ .gitignore 2>/dev/null; true \
            && git commit -m "auto-recover: evaluator session orphaned changes (${dirty_count} files)" \
            && git push; then
...
latest_summary=$(ls -t "$REPORTS_DIR"/evaluator-*.md 2>/dev/null | head -1)
