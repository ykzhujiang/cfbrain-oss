 ...
 *
 * CFBrain is agnostic about where files live. The setup skill picks
 * the backend (Supabase Storage or S3/R2/MinIO), cfbrain doesn't care.
 */
...
export async function createStorage(config: StorageConfig): Promise<StorageBackend> {
  switch (config.backend) {
    ...
    case 'local': {
      ...
      return new LocalStorage(config.localPath || '/tmp/cfbrain-storage');
    }
    default:
