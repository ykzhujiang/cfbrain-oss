...
function upgradeCommandForMethod(method: string): string {
  switch (method) {
    case 'bun': return 'bun update cfbrain';
    case 'clawhub': return 'clawhub update cfbrain';
    case 'binary': return 'Download from https://github.com/garrytan/cfbrain/releases';
    default: return 'cfbrain upgrade';
  }
}
...
async function fetchLatestRelease(): Promise<{ tag: string; published_at: string; url: string } | null> {
  try {
    const res = await fetch('https://api.github.com/repos/garrytan/cfbrain/releases/latest', {
      headers: { 'User-Agent': `cfbrain/${VERSION}` },
...
async function fetchChangelog(currentVersion: string, latestVersion: string): Promise<string> {
  try {
    const res = await fetch('https://raw.githubusercontent.com/garrytan/cfbrain/master/CHANGELOG.md', {
      signal: AbortSignal.timeout(10_000),
    });
    if (!res.ok) return '';
...
export async function runCheckUpdate(args: string[]) {
  if (args.includes('--help') || args.includes('-h')) {
    console.log('Usage: cfbrain check-update [--json]\n\nCheck for new CFBrain versions.\n\nOnly reports minor/major version bumps (v0.X.0), not patches.\nFails silently on network errors.');
    return;
  }
  ...
  if (!release) {
    if (json) {
      ...
      console.log(`CFBrain ${VERSION} — could not check for updates (no releases found or network unavailable).`);
    }
    return;
  }
  ...
  if (json) {
    ...
    console.log(`CFBrain update available: ${VERSION} → ${latestVersion}`);
    console.log(`Run: ${upgradeCmd}`);
    console.log(`Release: ${release.url}`);
  } else {
    console.log(`CFBrain ${VERSION} is up to date.`);
  }
}
