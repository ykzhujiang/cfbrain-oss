 ...
 *
 * Usage:
 *   cfbrain migrate --to supabase [--url <connection_string>]
 *   cfbrain migrate --to pglite [--path <db_path>]
 *   cfbrain migrate --to <engine> --force  (overwrite non-empty target)
 */
...
function parseArgs(args: string[]): MigrateOpts {
  ...
  if (toIdx === -1 || !args[toIdx + 1]) {
    throw new Error('Usage: cfbrain migrate --to <supabase|pglite> [--url <url>] [--path <path>] [--force]');
  }
...
function getManifestPath(): string {
  const newPath = join(homedir(), '.cfbrain', 'migrate-manifest.json');
  const oldPath = join(homedir(), '.gbrain', 'migrate-manifest.json');
  // Backward compat: if .cfbrain manifest doesn't exist but .gbrain does, use old path
...
export async function runMigrateEngine(sourceEngine: BrainEngine, args: string[]): Promise<void> {
  ...
  if (opts.targetEngine === 'postgres') {
    targetConfig.database_url = opts.targetUrl || process.env.CFBRAIN_DATABASE_URL || process.env.GBRAIN_DATABASE_URL || process.env.DATABASE_URL;
    if (!targetConfig.database_url) {
      console.error('Target is Supabase but no connection string provided. Use: --url <connection_string>');
      process.exit(1);
    }
  } else {
    targetConfig.database_path = opts.targetPath || join(homedir(), '.cfbrain', 'brain.pglite');
  }
  ...
  for (const page of pagesToMigrate) {
    ...
    // createVersion takes a snapshot of current state — after putPage above,
    // one snapshot captures the migrated content. Historical ordering is lost.
    if (versions.length > 0) {
      try {
        await targetEngine.createVersion(page.slug);
      } catch {
        // Non-fatal — some engines may not support versioning
      }
      if (versions.length > 1) {
        console.log(`  ⚠ ${page.slug}: ${versions.length} version(s) in source, only current state migrated (engine limitation)`);
      }
    }
